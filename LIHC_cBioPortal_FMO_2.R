...
genset<-read.csv("c:/Users/xjmik/Downloads/Flavin containing monooxygenases.csv",header = TRUE,sep = ",")
...
lihc_clinical$"FMO" <- gsva_gl[match(lihc_clinical$PATIENT_ID,gsva_gl$sample),][,"FMO"]


df<-subset(lihc_clinical,select =c("OS_STATUS","OS_MONTHS","DFS_STATUS","DFS_MONTHS","FMO","PFS_STATUS","PFS_MONTHS","DSS_STATUS","DSS_MONTHS"))
...
res.cut<-surv_cutpoint(df,time = "OS_MONTHS",event = "events",variables = c("FMO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(OS_MONTHS,events)~ FMO ,data = res.cat)
...
res.cut<-surv_cutpoint(df,time = "DFS_MONTHS",event = "events",variables = c("FMO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DFS_MONTHS,events)~ FMO ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = c("FMO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(PFS_MONTHS,events)~ FMO  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = c("FMO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DSS_MONTHS,events)~ FMO  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
