...
genset<-read.csv("c:/Users/xjmik/Downloads/Sulfotransferases.csv",header = TRUE,sep = ",")
genset<-as.data.frame(genset$Approved.symbol)
colnames(genset)<-"Sulf"
...
lihc_clinical$"Sulf" <- gsva_gl[match(lihc_clinical$PATIENT_ID,gsva_gl$sample),][,"Sulf"]


df<-subset(lihc_clinical,select =c("OS_STATUS","OS_MONTHS","DFS_STATUS","DFS_MONTHS","Sulf","PFS_STATUS","PFS_MONTHS","DSS_STATUS","DSS_MONTHS"))
...
res.cut<-surv_cutpoint(df,time = "OS_MONTHS",event = "events",variables = c("Sulf"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(OS_MONTHS,events)~ Sulf ,data = res.cat)
...
res.cut<-surv_cutpoint(df,time = "DFS_MONTHS",event = "events",variables = c("Sulf"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DFS_MONTHS,events)~ Sulf ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = c("Sulf"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(PFS_MONTHS,events)~ Sulf  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = c("Sulf"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DSS_MONTHS,events)~ Sulf  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
