...
genset<-read.table("c:/Users/xjmik/Downloads/epoxide hydrolase.txt",sep = "\t")
...
lihc_clinical$"EPO" <- gsva_gl[match(lihc_clinical$PATIENT_ID,gsva_gl$sample),][,"EPO"]


df<-subset(lihc_clinical,select =c("OS_STATUS","OS_MONTHS","DFS_STATUS","DFS_MONTHS","EPO","PFS_STATUS","PFS_MONTHS","DSS_STATUS","DSS_MONTHS"))
...
res.cut<-surv_cutpoint(df,time = "OS_MONTHS",event = "events",variables = c("EPO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(OS_MONTHS,events)~ EPO ,data = res.cat)
...
res.cut<-surv_cutpoint(df,time = "DFS_MONTHS",event = "events",variables = c("EPO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DFS_MONTHS,events)~ EPO ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = c("EPO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(PFS_MONTHS,events)~ EPO  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = c("EPO"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DSS_MONTHS,events)~ EPO  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
