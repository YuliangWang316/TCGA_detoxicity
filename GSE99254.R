...
write.table(pbmc.marker,"C:/Users/xjmik/Desktop/GSE99254_Totalmarker.txt",sep = "\t")
remove(pbmc.marker)
...
remove(i)
for (i in levels(Idents(pbmc))[c(1:14,17,19:23)]) {
  ...
  write.table(pbmcs.markers,file = paste0("c:/Users/xjmik/Desktop/GSE99254_T_N",i,".txt"),sep = "\t")
}
remove(i,pbmcs,pbmcs.markers)
for (i in levels(Idents(pbmc))[c(1:15,17,19:21,23)]) {
  pbmcs<-subset(pbmc,idents = i)
  Idents(pbmcs)<-pbmcs@meta.data$Bulksample
  pbmcs.markers<-FindMarkers(pbmcs,only.pos = TRUE,ident.1 = "T",ident.2 = "P")
  write.table(pbmcs.markers,file = paste0("c:/Users/xjmik/Desktop/GSE99254_T_P",i,".txt"),sep = "\t")
...
write.table(pbmc.marker2,"C:/Users/xjmik/Desktop/GSE99254_BulkTotalmarkers.txt",sep = "\t")
remove(pbmc.marker2)
for (i in levels(Idents(pbmc))) {
  pbmcs<-subset(pbmc,idents = i)
  Idents(pbmcs)<-pbmcs@meta.data$Bulksample
  pbmcs.markers<-FindMarkers(pbmcs,only.pos = TRUE,ident.1 = "T",ident.2 = "N")
  write.table(pbmcs.markers,file = paste0("c:/Users/xjmik/Desktop/GSE99254_Bulk_T_N",i,".txt"),sep = "\t")
}
remove(i,pbmcs,pbmcs.markers)
for (i in levels(Idents(pbmc))) {
  pbmcs<-subset(pbmc,idents = i)
  Idents(pbmcs)<-pbmcs@meta.data$Bulksample
  pbmcs.markers<-FindMarkers(pbmcs,only.pos = TRUE,ident.1 = "T",ident.2 = "P")
  write.table(pbmcs.markers,file = paste0("c:/Users/xjmik/Desktop/GSE99254_Bulk_T_P",i,".txt"),sep = "\t")
  remove(pbmcs.markers)
}
remove(i,pbmcs)
...
pbmc<-RunUMAP(pbmc,reduction = "lda",reduction.name = "lda_umap",dims = 1:24)
pbmc<-RunTSNE(pbmc,reduction = "lda",reduction.name = "lda_tsne",dims = 1:24)
Idents(pbmc)<-pbmc@meta.data$majorCluster
DimPlot(pbmc,reduction = "lda_umap")
DimPlot(pbmc,reduction = "lda_tsne")
pbmc<-RunUMAP(pbmc,reduction = "lda_bulksample2",reduction.name = "lda_bulksample2_umap",dims = 1:3)
pbmc<-RunTSNE(pbmc,reduction = "lda_bulksample2",reduction.name = "lda_bulksample2_tsne",dims = 1:3)
Idents(pbmc)<-pbmc@meta.data$Bulksample2
DimPlot(pbmc,reduction = "lda_bulksample2_umap")
DimPlot(pbmc,reduction = "lda_bulksample2_tsne")
saveRDS(pbmc,file = "c:/Users/xjmik/Desktop/GSE99254pbmc.rds")
DimPlot(pbmc,reduction = "lda_tsne")
Idents(pbmc)<-pbmc@meta.data$majorCluster
...
Tregpbmc<-subset(pbmc,idents = c("CD4_C8-FOXP3"))
...
write.table(pbmc.marker,"c:/Users/xjmik/Desktop/pbmcmarkers_2.txt",sep = "\t")
pbmcmarkers<-read.table("c:/Users/xjmik/Desktop/pbmcmarkers_2.txt",sep = "\t",header = TRUE,row.names = 1)
...
remove(data,fd,fData,pd,pbmcmarkers,pbmcmarkers_new,pbmcmarkers_new_new,pbmc.marker,ordering_genes)

Tregtumor<-subset(pbmc_raw,idents = c("CD4_C9-CTLA4"))
Tregpbmc<-subset(pbmc_raw,idents = c("CD4_C8-FOXP3"))
Tregpbmc.data<-Tregpbmc@assays$RNA@counts
remove(Tregpbmc)
JMJD1C<-FetchData(Tregtumor,vars = "JMJD1C")
JMJD1C$sample<-rep("jmjd1c",length(rownames(JMJD1C)))
...
Tregtumor<-subset(pbmc_raw,idents = c("CD4_C9-CTLA4"))
CD8<-subset(pbmc_raw,idents = c("CD8_C2-CD28","CD8_C3-CX3CR1","CD8_C1-LEF1","CD8_C7-SLC4A10","CD8_C4-GZMK","CD8_C5-ZNF683","CD8_C6-LAYN"))
Idents(CD8)<-CD8@meta.data$Bulksample
CD8tumor<-subset(CD8,idents = "T")
remove(CD8)
CD4<-subset(pbmc_raw,idents = c("CD4_C4-CD69","CD4_C6-GZMA","CD4_C3-GNLY","CD4_C1-CCR7","CD4_C8-FOXP3","CD4_C2-ANXA1","CD4_C5-EOMES","CD4_C9-CTLA4","CD4_C7-CXCL13","CD4.MAIT","CD4.iNKT"))
Idents(CD4)<-CD4@meta.data$Bulksample
CD4tumor<-subset(CD4,idents = "T")
remove(CD4)
Idents(Tregtumor)<-Tregtumor$Patient
Idents(CD8tumor)<-CD8tumor$Patient
Idents(CD4tumor)<-CD4tumor$Patient
patient<-unique(Tregtumor@meta.data$Patient)
