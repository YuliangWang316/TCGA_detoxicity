...
genset<-read.table("c:/Users/xjmik/Downloads/Peroxidase_new.txt",header = TRUE,sep = "\t")
genset<-as.data.frame(genset$Symbol)
colnames(genset)<-"Peroxidase_signature"
...
lihc_clinical$"Peroxidase_signature" <- gsva_gl[match(lihc_clinical$PATIENT_ID,gsva_gl$sample),][,"Peroxidase_signature"]


df<-subset(lihc_clinical,select =c("OS_STATUS","OS_MONTHS","DFS_STATUS","DFS_MONTHS","Peroxidase_signature","PFS_STATUS","PFS_MONTHS","DSS_STATUS","DSS_MONTHS"))

df <- df[!is.na(df$Peroxidase_signature),]
...
res.cut<-surv_cutpoint(df,time = "OS_MONTHS",event = "events",variables = c("Peroxidase_signature"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(OS_MONTHS,events)~ Peroxidase_signature ,data = res.cat)
...
res.cut<-surv_cutpoint(df,time = "DFS_MONTHS",event = "events",variables = c("Peroxidase_signature"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DFS_MONTHS,events)~ Peroxidase_signature ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = c("Peroxidase_signature"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(PFS_MONTHS,events)~ Peroxidase_signature  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = c("Peroxidase_signature"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DSS_MONTHS,events)~ Peroxidase_signature  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
