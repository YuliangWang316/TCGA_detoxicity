...
genset<-read.csv("c:/Users/xjmik/Downloads/Aldehyde dehydrogenases.csv",header = TRUE,sep = ",")
genset<-as.data.frame(genset$Approved.symbol)
colnames(genset)<-"Aldehyde_dehydrogenases"
...
lihc_clinical$"Aldehyde_dehydrogenases" <- gsva_gl[match(lihc_clinical$PATIENT_ID,gsva_gl$sample),][,"Aldehyde_dehydrogenases"]


df<-subset(lihc_clinical,select =c("OS_STATUS","OS_MONTHS","DFS_STATUS","DFS_MONTHS","Aldehyde_dehydrogenases","PFS_STATUS","PFS_MONTHS","DSS_STATUS","DSS_MONTHS"))

df <- df[!is.na(df$Aldehyde_dehydrogenases),]
...
res.cut<-surv_cutpoint(df,time = "OS_MONTHS",event = "events",variables = c("Aldehyde_dehydrogenases"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(OS_MONTHS,events)~ Aldehyde_dehydrogenases ,data = res.cat)
...
res.cut<-surv_cutpoint(df,time = "DFS_MONTHS",event = "events",variables = c("Aldehyde_dehydrogenases"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DFS_MONTHS,events)~ Aldehyde_dehydrogenases ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = c("Aldehyde_dehydrogenases"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(PFS_MONTHS,events)~ Aldehyde_dehydrogenases  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = c("Aldehyde_dehydrogenases"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DSS_MONTHS,events)~ Aldehyde_dehydrogenases  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
