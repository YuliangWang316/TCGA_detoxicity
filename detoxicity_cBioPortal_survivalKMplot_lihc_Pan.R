...
remove(samplesTP,d)
setwd("c:/Users/xjmik/Desktop/Detoxicity_2/LIHC_tcga_pan_OS/")
for (i in c(58:72,74:208,211:256,258:302,304:366,368:393,395:length(colnames(lihc_clinical)))) {
...
setwd("c:/Users/xjmik/Desktop/Detoxicity_2/LIHC_tcga_pan_DFS/")
...
remove(df,fit,p,res.cat,res.cut,i,cutoff)
setwd("c:/Users/xjmik/Desktop/Detoxicity_2/LIHC_tcga_pan_PFS/")

for (i in c(58:72,74:208,211:256,258:302,304:366,368:393,395:length(colnames(lihc_clinical)))) {
  df<-subset(lihc_clinical,select =c("PFS_STATUS","PFS_MONTHS","DSS_STATUS","DSS_MONTHS",colnames(lihc_clinical)[i]))
  ...
  res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = colnames(df)[5])
  summary(res.cut)
  res.cat<-surv_categorize(res.cut)
  for (j in 1:length(rownames(res.cat))) {
    if(res.cat[,3][j] == "high"){
  ...
  remove(j)
  cutoff<-res.cat[,3]
  fit<-survfit(Surv(PFS_MONTHS,events)~ cutoff ,data = res.cat)
  
  p<-ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE,)
  pdf(file=paste(colnames(lihc_clinical)[i],"_PFS.pdf",sep=""))
  print(p,newpage = FALSE)
  dev.off()
}

remove(df,fit,p,res.cat,res.cut,i,cutoff)
setwd("c:/Users/xjmik/Desktop/Detoxicity_2/LIHC_tcga_pan_DSS/")
...
for (i in c(58:72,74:208,211:256,258:302,304:366,368:393,395:length(colnames(lihc_clinical)))) {
  ...
  res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = colnames(df)[5])
  summary(res.cut)
  res.cat<-surv_categorize(res.cut)
  for (j in 1:length(rownames(res.cat))) {
    if(res.cat[,3][j] == "high"){
  ...
  remove(j)
  cutoff<-res.cat[,3]
  fit<-survfit(Surv(DSS_MONTHS,events)~ cutoff ,data = res.cat)
  
  p<-ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE,)
  pdf(file=paste(colnames(lihc_clinical)[i],"_DSS.pdf",sep=""))
  print(p,newpage = FALSE)
  dev.off()
}
remove(df,fit,p,res.cat,res.cut,i,cutoff)

df<-subset(lihc_clinical,select =c("OS_STATUS","OS_MONTHS","DFS_STATUS","DFS_MONTHS","AKR1B10","PFS_STATUS","PFS_MONTHS","DSS_STATUS","DSS_MONTHS"))

df <- df[!is.na(df$AKR1B10),]
...
library(survminer)
df$OS_MONTHS<-as.numeric(df$OS_MONTHS)
res.cut<-surv_cutpoint(df,time = "OS_MONTHS",event = "events",variables = c("AKR1B10"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(OS_MONTHS,events)~ AKR1B10 ,data = res.cat)
...
library(survminer)
df$DFS_MONTHS<-as.numeric(df$DFS_MONTHS)
res.cut<-surv_cutpoint(df,time = "DFS_MONTHS",event = "events",variables = c("AKR1B10"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DFS_MONTHS,events)~ AKR1B10 ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
library(survminer)
df$PFS_MONTHS<-as.numeric(df$PFS_MONTHS)
res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = c("AKR1B10"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(PFS_MONTHS,events)~ AKR1B10  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
library(survminer)
df$DSS_MONTHS<-as.numeric(df$DSS_MONTHS)
res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = c("AKR1B10"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DSS_MONTHS,events)~ AKR1B10  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
library(survminer)
df$DFS_MONTHS<-as.numeric(df$DFS_MONTHS)
res.cut<-surv_cutpoint(df,time = "DFS_MONTHS",event = "events",variables = c("AKR1C3"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DFS_MONTHS,events)~ AKR1C3 ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
library(survminer)
df$PFS_MONTHS<-as.numeric(df$PFS_MONTHS)
res.cut<-surv_cutpoint(df,time = "PFS_MONTHS",event = "events",variables = c("AKR1C3"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(PFS_MONTHS,events)~ AKR1C3  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
...
library(survminer)
df$DSS_MONTHS<-as.numeric(df$DSS_MONTHS)
res.cut<-surv_cutpoint(df,time = "DSS_MONTHS",event = "events",variables = c("AKR1C3"))
summary(res.cut)
res.cat<-surv_categorize(res.cut)
fit<-survfit(Surv(DSS_MONTHS,events)~ AKR1C3  ,data = res.cat)
ggsurvplot(fit,data = res.cat,risk.table = TRUE,pval = TRUE)
