...
hi<-rownames(res.cat[which(res.cat$AKR1B10 == "high"),])
lo<-rownames(res.cat[which(res.cat$AKR1B10 == "low"),])
remove(samplesTP)
...
condition<-factor(c(rep("hi",38),rep("lo",322)),levels = c("lo","hi"))
library(DESeq2)
...
head(res)
summary(res)
write.csv(res,file="c:/Users/xjmik/Desktop/Detoxicity_2/AKR1B10_survival_hilo.csv")
remove(colData,dds,res,condition)
write.table(lihc_gene,"c:/Users/xjmik/Desktop/Detoxicity_2/AKR1B10_survival_hilo_forGSEA.txt",sep = "\t")
v <-voom(count, plot = F, save.plot = F)
out=v$E
out=rbind(ID=colnames(out),out)
write.table(out,file="AKR1B10survivalhilo_count.txt",sep="\t",quote=F,col.names=F)  
source("TMEimmune31.CIBERSORT.R")
results=CIBERSORT("ref.txt", "AKR1B10survivalhilo_count.txt", perm=100, QN=TRUE)
remove(v,out,results)
results=CIBERSORT("ref.txt", "AKR1B10survivalhilo_cBioPortalgene.txt", perm=100, QN=TRUE)
remove(v,out,results)
